...
JOINTS.SetA.amean=(-10-10).*rand(1,1)+10;
...
JOINTS.SetB.amean=(100-80).*rand(1,1)+80;
%JOINTS.SetB.angle=normrnd(JOINTS.SetB.amean,JOINTS.SetB.astandard,JOINTS.SetB.njoint,1);
% 2D joint point generation (Poisson process)
...
MEANPERS=(1/JOINTS.SetA.onedimfreq*1/JOINTS.SetB.onedimfreq)/(sind(abs(DIP_SetB-DIP_SetA)));
MEANNONPERS=mean(SurfArea);
DIFF=MEANPERS/MEANNONPERS;
GAPA=(1/FREQUENCY_SetB)/(sind(abs(DIP_SetB-DIP_SetA)));
GAPB=(1/FREQUENCY_SetA)/(sind(abs(DIP_SetB-DIP_SetA)));
PERSFACTORA=TRACELENGTH_SetA/GAPA;
PERSFACTORB=TRACELENGTH_SetB/GAPB;
PERSFACMEAN=sqrt(PERSFACTORA*PERSFACTORB);
surfareatable=table(sort(SurfArea)');
isbdtable=table(ISBD_Wang,ISBD_NONPERSISTENT,ISBD_MULTIPLICATOR);
infotable=table(PERSFACMEAN,DIFF,PERSFACTORA,PERSFACTORB,MEANPERS,MEANNONPERS,SIZE_of_ANALYSIS,DIP_SetA,STD_of_DIP_SetA,TRACELENGTH_SetA,FREQUENCY_SetA,DIP_SetB,STD_of_DIP_SetB,TRACELENGTH_SetB,FREQUENCY_SetB);
fittingtable=table(X,Y,Z);
filepath='D:/ISBDMATLAB/isbdinfo%s';
filexls='.xls';
%filename=sprintf(filepath,index,filexls);

filename2=[genvarname(['adapt', num2str(datestr8601(now,'ymdHMS'))])];
